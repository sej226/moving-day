...
<v-card style="width:300px; margin-left:5%;" outlined>
    ...
    <v-card-title v-if="value._links">
        Moving # {{value._links.self.href.split("/")[value._links.self.href.split("/").length - 1]}}
    </v-card-title >
    <v-card-title v-else>
        Moving
    </v-card-title >
    ...
    <v-card-text style = "margin-left:-15px; margin-top:10px;">
          ...
          <div class="grey--text ml-4" v-if="editMode" style = "margin-top:-20px;">
            <v-text-field type="number" label="MovingId" v-model="value.movingId"/>
          </div>
          <div class="grey--text ml-4" v-else>
            MovingId :  {{value.movingId }}
          </div>
          <div class="grey--text ml-4" v-if="editMode" style = "margin-top:-20px;">
            <v-text-field label="PhoneNumber" v-model="value.phoneNumber"/>
          </div>
          <div class="grey--text ml-4" v-else>
          ...
          <div class="grey--text ml-4" v-if="editMode" style = "margin-top:-20px;">
            <v-text-field label="MovingStatus" v-model="value.movingStatus"/>
          </div>
          <div class="grey--text ml-4" v-else>
            MovingStatus :  {{value.movingStatus }}
          </div>
    ...
    methods: {
      ...
      async save(){
        try{
          ...
          if(this.isNew){
            temp = await axios.post(axios.fixUrl('/movings'), this.value)
          }else{
